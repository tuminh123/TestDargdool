...
{
    [Header("Torque (lực xoay)")][Tooltip("Lực torque tác động lên tay")] 
    ...
    [Header("Recovery")][Header("Translation Force")] 
...
{ 
    ...
    { 
        float elapsed = 0f; 
        try { // Áp lực trong một khoảng thời gian ngắn
        while (elapsed < profile.impulseDuration) 
            { 
                ...
                { if (item == null) continue; 
                    ...
                    item.rb.AddTorque(intent.Sign * profile.armTorque, ForceMode2D.Impulse); } await UniTask.WaitForFixedUpdate(token); 
                } 
        } catch { } } 
...
{
    [SerializeField] AttackHandle[] attackHandles; [SerializeField] BalanceTest[] balanceTests; private CancellationTokenSource cts; public AttackHandle currentAttack { get; private set; }
